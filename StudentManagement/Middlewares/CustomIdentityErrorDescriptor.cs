...
namespace StudentManagement.Middlewares
{
    public class CustomIdentityErrorDescriptor : IdentityErrorDescriber
    {
        ...
        {
            return new IdentityError { Code = nameof(PasswordTooShort), Description = $"密码长度不小于 {length} 个字符！" };
        }
        ...
        {
            return new IdentityError { Code = nameof(PasswordRequiresUniqueChars), Description = $"密码必须使用至少不同的 {uniqueChars} 个字符！" };
        }
